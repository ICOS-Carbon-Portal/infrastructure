...
def list_slots(station, root=OLD_ROOT):
    ...
    if n:
        debug(
            "There are %s foot files in %s with no corresponding"
            " .RData or .RDatafoot files" % (n, fp_dir)
        )
    if rd_files:
        debug(
            "There are %s .RData files in %s with no corresponding"
...
def sync_single_station(station, old_root=OLD_ROOT, new_root=NEW_ROOT):
    ...
    for slot in list_slots(station, old_root):
        ...
        try:
            ...
            debug("No csv for station %s and slot '%s'" % (station, slot.foot.date))
            continue
