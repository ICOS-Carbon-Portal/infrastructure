import { bbserver_home } from "../_ctx.ts";
import { type TaskFile } from "../../../lib/ansible/play.ts";
import { bbserver_user } from "../../../lib/globals.ts";
import { randomInt, tmpl } from "../../../lib/template.ts";

export default [
  {
    name: tmpl`Create ${bbserver_home}/bin directory`,
    file: {
      path: tmpl`${bbserver_home}/bin`,
      state: "directory",
      owner: bbserver_user,
      group: bbserver_user,
    },
  },
  {
    name: "Copy bbserver.py",
    template: {
      src: "bbserver.py",
      mode: "+x",
      dest: tmpl`${bbserver_home}/bin/bbserver`,
    },
  },
  {
    name: "Prime borg cache by running 'bbserver list' each night",
    cron: {
      user: "bbserver",
      job: tmpl`${bbserver_home}/bin/bbserver list > /dev/null 2>&1`,
      hour: randomInt(4, "bbserver"),
      minute: randomInt(60, "bbserver"),
      name: "bbserver_prime_borg_cache",
    },
  },
] satisfies TaskFile;
