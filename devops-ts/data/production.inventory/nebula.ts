...
const { V, tmpl } = context<Globals & BuiltinVars>();
...
export default {
  "nebula_hosts": {
    "vars": {
      ...
      "nebula_ping_host": tmpl`fsicos4.${V.nebula_interface}`,
...
${jinja`{{ ${V.hostvars}[host]['nebula_ip'] }}`}	${jinja`{{ ${V.hostvars}[host]['inventory_hostname_short'] }}`}.nebula
${jinja`{% endfor %}`}
`,
    },
    "hosts": {
