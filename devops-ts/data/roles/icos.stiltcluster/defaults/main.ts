// Auto-generated by gen-data.ts from ../devops/roles/icos.stiltcluster/defaults/main.yml
import type { VarsFile } from "../../../../lib/data.ts";
import { expr, tmpl } from "../../../../lib/ansible.ts";

export default {
  "stiltcluster_username": "stiltcluster",
  "stiltcluster_servicename": "stiltcluster",
  "stiltcluster_home": "/home/stiltcluster",
  "stiltcluster_bindir": tmpl`${expr("stiltcluster_home")}/bin`,
  "stiltcluster_fetch_host": null,
  "stiltcluster_fetch_path": tmpl`${
    expr(
      "hostvars[stiltcluster_fetch_host].stiltcluster_home\n  | default(stiltcluster_home)",
    )
  }/stiltcluster.jar"`,
  "stiltcluster_hostname": expr("inventory_hostname"),
  "stiltcluster_stiltweb_hostname": expr("inventory_hostname"),
  "stiltcluster_stiltweb_port": 2550,
  "stiltcluster_port": 2561,
  "stiltcluster_maxcores": 0,
  "stiltcluster_docker": true,
} satisfies VarsFile;
