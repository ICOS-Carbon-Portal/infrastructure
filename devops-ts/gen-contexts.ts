 ...
 * V from lib/vars.ts), plus `isDef("<name>")` / `notVar("<name>")` (always
 * _ctx exports).
 */
...
for (const [roleName, vars] of roleVars) {
  ...
  // The inferred Vars types may reference `Tmpl` (lib/template.ts) or `VarValue`
  // (lib/ansible/values.ts); import whichever the interface actually uses,
  // independent of the context helpers (a role with no context() still has a
  // typed Vars its data satisfies).
