...
for (const [roleName, vars] of roleVars) {
  ...
  // Emit a named binding per OWN variable, so a role's task files import them
  // the normal TS way (`import { nexus_home } from "../_ctx.ts"`) rather than
  // through a `V` proxy. Shaped names (lib/shapes.ts) are owned by that module
  // and not re-exported here. The Vars interface stays as the type this role's
  // generated data `satisfies`; its fields may be `Tmpl`/`VarValue`.
  const allTypes = [...vars.values()].join(" ");
  const usesType = (t: string) => new RegExp(`\\b${t}\\b`).test(allTypes);
  const bindNames = [...vars.keys()]
    .filter((n) => ident.test(n) && !SHAPES.has(n))
    .sort((a, b) => a.localeCompare(b));

  const fromTemplate = [
    usesType("Tmpl") ? "type Tmpl" : "",
    bindNames.length ? "type VarRef" : "",
    bindNames.length ? "varProxy" : "",
  ].filter(Boolean);
  ...
  const bindings = bindNames.length
  ...
  const out = `// Auto-generated by gen-contexts.ts from\n` +
    ...
    `// Per-role variable context: ${vars.size} own variables, each exported as\n` +
