...
import { parseJinja } from "./lib/jinja-parse.ts";
...
for (const rel of await dataYmls()) {
  ...
  const helperImport = used.size
    ? `import { ${[...used].sort().join(", ")} } from "${ups}lib/ansible.ts";\n`
    : "";
  ...
  /** A templated string -> structured expr/tmpl/rawTmpl (data files have no V context). */
