...
for (const rel of await dataYmls()) {
  ...
  const tmplImport = usedTmpl
    ? `import { tmpl } from "${ups}lib/ansible.ts";\n`
    : "";
  ...
  // Serialize like JSON, but wrap Jinja-template strings in tmpl(...) so they
  // render as quoted scalars (matching the role/playbook quoting).
  function emit(v: unknown, indent: string): string {
    if (typeof v === "string") {
      return (v.includes("{{") || v.includes("{%"))
        ? ((usedTmpl = true), `tmpl(${JSON.stringify(v)})`)
        : JSON.stringify(v);
    }
