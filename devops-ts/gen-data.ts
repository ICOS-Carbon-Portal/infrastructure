...
for (const rel of await dataYmls()) {
  ...
  const helperImports = (names: string[]): string => {
    const byMod = new Map<string, string[]>();
    for (const n of names) {
      const mod = HELPER_MODULE[n];
      (byMod.get(mod) ?? byMod.set(mod, []).get(mod)!).push(n);
    }
    return [...byMod.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([mod, ns]) =>
        `import { ${ns.sort().join(", ")} } from "${ups}lib/${mod}";\n`
      )
      .join("");
  };

  // Header. When V is used, source the template helpers from a per-file context
  // (so there's a single `V`/`tmpl`); otherwise import the few helpers used
  // directly from their lib submodules.
  let header = `// Auto-generated by gen-data.ts from ../devops/${rel}\n` +
    (varsImport ?? `import type { ${name} } from "${ups}lib/data.ts";\n`);
  if (used.has("register")) {
    header += helperImports(["register"]);
    used.delete("register");
  }
  if (used.has("V")) {
    // Value-builder primitives live in lib submodules, not the per-file
    // context() (which provides only V/tmpl). Import any that are used.
    const ANSIBLE = [
      "concat",
      "eq",
      "hostvar",
    ...
    if (fromAnsible.length) {
      header += helperImports(fromAnsible);
      for (const h of fromAnsible) used.delete(h);
    }
    const helpers = [
      "V",
      ...["tmpl"].filter((h) => used.has(h)),
    ];
    ...
    header += helperImports([...used]);
  }
  // Registry imports for restated globals/builtins, when no V context emitted
  // them above (a defaults file of pure values that names a global/builtin).
  if (!used.has("V")) {
    if (restateGlobals) {
      header += `import type { Globals } from "${ups}lib/globals.ts";\n`;
    }
    if (restateBuiltins) {
      header += `import type { BuiltinVars } from "${ups}lib/builtins.ts";\n`;
    }
  }
  const out =
    `${header}\nexport default ${literal} satisfies ${satisfiesType};\n`;
