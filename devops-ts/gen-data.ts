...
for (const rel of await dataYmls()) {
  ...
  // A role data file reaches its role's own vars through that role's `V`.
  let needRoleV = false;
  if (used.has("V")) {
    used.delete("V");
    if (used.has("tmpl")) {
    ...
    // own-role > globals > builtins > params > vault > shared. A role's OWN vars
    // are reached through that role's `V` accessor; the rest are named imports
    // from their owning module (object shapes are typed there as objects).
    const usedNames = [
      ...new Set(
    ...
    if (needRoleV) addImp(`${ups}roles/${dataRole}/_ctx.ts`, "V");
  } else {
    for (const h of used) {
      if (HELPER_MODULE[h]) addImp(`${ups}lib/${HELPER_MODULE[h]}`, h);
    }
  ...
  const localDecls = localSelf.length
    ? "\n" +
      ...
      "\n"
    : "";
  // Role-own vars keep their `V.` prefix (reached through the role's `V`); every
  // other ref is imported by name, so its `V.` prefix is dropped.
  const body = literal.replace(
    /\bV\.([A-Za-z_]\w*)/g,
    (m, n) => (dataRole && selfVarSet.has(n)) ? m : n,
  );
