...
## Final coverage of all 683 devops `.yml`

`verify` proves **628** TS modules render semantically identical to their YAML:
- 71 playbooks (67 top-level + 4 `fixes/`)
- 397 role task/handler files
- 160 data modules (`data/` mirror: role meta/defaults/vars, inventories,
  host_vars/group_vars) — generated by `gen-data.ts`, typed via `lib/data.ts`
  (meta deps check role names + tags).

The remaining 55 are deliberately not ported:
- **26 vault files** — `$ANSIBLE_VAULT` ciphertext (25 whole-file + 1 inline
  `!vault`). Encrypted secrets, not data; must stay `.yml`.
- **27 app payloads** — `roles/*/{files,templates}/*.yml` are Docker-Compose /
  schema / Jinja templates copied or templated verbatim, not Ansible structures.
- **2 empty** — comments-only task files (parse to null).
