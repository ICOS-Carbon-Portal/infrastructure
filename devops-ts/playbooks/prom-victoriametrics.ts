...
export default [
  {
    ...
    roles: [
      ...
      role("icos.victoriametrics", {
        ...
        vm_graf_pass: tmpl("{{ vault_vm_graf_pass }}"),
        vm_promlens_token: tmpl("{{ vault_prometheus_promlens_token }}"),
      }).tags("prom"),
      role("icos.caddy", {
    ...
    tasks: [
      {
        ...
        debug: {
          msg: tmpl(
            "Basic auth for prometheus is {{ vault_vmagent_auth.username }}/{{ vault_vmagent_auth.password }}",
          ),
        },
      },
    ],
