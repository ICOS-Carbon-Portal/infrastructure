  ...
  // ### ICOS1 ###
  {
    ...
    pre_tasks: [
      ...
      // RDFLOG
      {
        ...
        "ansible.posix.synchronize": {
          owner: false,
          group: false,
          src: tmpl`${expr("local_tmp")}/rdflog_dump.gz`,
          dest: "/tmp/",
        },
      },
      // CPMETA
      // metaAppStorage doesn't compress very well, so just rsync the directory
      {
        ...
        "ansible.posix.synchronize": {
          src: tmpl`${expr("local_tmp")}/metaAppStorage`,
          dest: "/home/cpmeta/",
          // Create local directories below dest directory
          rsync_opts: ["--mkpath"],
    ...
    roles: [
      ...
      role("icos.rdflog", {
        ...
        rdflog_restore_file: tmpl`${expr("local_tmp")}/rdflog_dump.gz`,
      }).tags("rdflog"),
