...
if(cpTF){
    if(!file.exists(paste(pathResults,"Footprints/",sep="")))dir.create(path=paste(pathResults,"Footprints/",sep=""), showWarnings = TRUE, recursive = FALSE)
    try(imagell(xfoot,lg=T,zlims=zlims,zoom=NULL,center=id2pos(identname)[c("lon","lat")],rays=F,lon.ll,lat.ll,lon.res,lat.res,nlevel=64,citiesTF=F,part=NULL,
            dev=paste(pathResults,"Footprints/",identname,".png",sep=""),type="foot",col=rev(heat.colors(64)),res.png=144,map.title=map.title))
} else {
