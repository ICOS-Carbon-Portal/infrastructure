...
in  [
    ...
    in  [
        Role::{ role = "icos.lxd_vm", lxd_vm_name = Some "build", lxd_vm_docker = Some True }
    ]
    }
  , Play::{
      hosts = "build",
  ...
  , Play::{
    ...
    in  [
      ...
      tasks = Some (let Task =
        ...
        , apt : Optional ({ name : List Text, update_cache : Optional Bool })
        , user : Optional ({ name : Text, shell : Text })
    ...
    in  [
      ...
      , Task::{ name = "Create build user", user = Some { name = "build", shell = "/bin/bash" } }
      , Task::{
      ...
      , Task::{ name = "Install nodejs", apt = Some { name = [ "nodejs" ], update_cache = Some True } }
    ])
    }
]
