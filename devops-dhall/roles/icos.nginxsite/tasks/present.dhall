-- Auto-generated from ../../../../devops/roles/icos.nginxsite/tasks/present.yml
...
in  [
    Task::{
      ...
      include_role = Some (Task.Poly_include_role.Record {
          apply = Some { tags = "nginxsite_cert" },
          name = "icos.certbot2",
  ...
  , Task::{
      ...
      include_role = Some (Task.Poly_include_role.Record {
          apply = Some { tags = "nginxsite_users" },
          name = "icos.nginxauth",
          tasks_from = None Text,
          public = Some True
      }),
  ...
  , Task::{
    ...
    in  [
      ...
      , Task::{
          name = Some "Remove old config file from sites-available",
          file = Some {
            path = None Text
          , state = "absent"
          , mode = None Text
          , dest = Some "{{ nginxsite_path_available }}"
          , recurse = None Bool
          , owner = None Text
      ...
      , Task::{
          name = Some "Remove old symlink sites-enabled",
          file = Some {
            path = None Text
          , state = "absent"
          , mode = None Text
          , dest = Some "{{ nginxsite_path_enable }}"
          , recurse = None Bool
          , owner = None Text
          , modification_time = None Text
          , access_time = None Text
      ...
      , Task::{ name = Some "Check syntax", changed_when = Some "False", shell = Some "nginx -t" }
      , Task::{
          name = Some "Reload nginx",
          changed_when = Some "False",
          systemd = Some { name = "nginx", state = Some "reloaded" }
        }
    ]),
    ...
    in  [
      ...
      , Task::{
          ...
          when = Some "update.backup_file is not defined",
    ...
    in  [
        Entry::{
          ...
          file = Some { name = None Text, state = "absent", path = Some "{{ update.backup_file }}" },
          changed_when = Some False,
          when = Some "update.backup_file is defined"
        }
    ])
