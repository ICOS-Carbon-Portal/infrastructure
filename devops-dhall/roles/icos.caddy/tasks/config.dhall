-- Auto-generated from ../../../../devops/roles/icos.caddy/tasks/config.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Add caddy configuration block",
          blockinfile = Some {
            create = True
          , path = "/etc/caddy/Caddyfile"
          , marker = "# {mark} {{ marker }}"
          , block = "{{ block }}"
          , state = Some "{{ state | default(omit) }}"
          , backup = Some True
          , insertafter = Some "{{ 'EOF' if where == 'EOF' else omit }}"
          , insertbefore = Some "{{ 'BOF' if where == 'BOF' else omit }}"
          , mode = None Natural
        },
          register = Some "_r"
        }
    ...
    in  [
        Entry::{
          ...
          file = Some { name = Some "{{ _r.backup_file }}", state = "absent", path = None Text },
