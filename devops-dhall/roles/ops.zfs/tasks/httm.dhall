-- Auto-generated from ../../../../devops/roles/ops.zfs/tasks/httm.yml
...
in  [
  ...
  , Task::{
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of httm",
          register = Some "gh",
          github_release = Some { user = "kimono-koans", repo = "httm", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set httm_version fact",
          set_fact = Some {
          ...
          , httm_version = Some "{{ gh.tag.lstrip('v') }}"
        }
        }
    ])
    }
  , Task::{
      ...
      apt = Some {
        ...
        upgrade = None Text,
        deb = Some "{{ httm_url_map[httm_architecture] }}",
        purge = None Bool,
        autoclean = None Bool,
  ...
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(httm_version)")
    }
  , Task::{
      name = Some "Which version of httm was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ httm_version }}" })
    }
]
