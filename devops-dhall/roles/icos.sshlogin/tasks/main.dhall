-- Auto-generated from ../../../../devops/roles/icos.sshlogin/tasks/main.yml
...
in  [
  ...
  , Task::{
      ...
      set_fact = Some (Task.Poly_set_fact.Record {
          ...
          sshlogin_src_user = Some "{{ sshlogin_user }}",
          sshlogin_dst_user = Some "{{ sshlogin_user }}",
  ...
  , Task::{
      ...
      loop = Some (Task.Poly_loop.Texts [ "sshlogin_dst", "sshlogin_src_user", "sshlogin_dst_user" ])
    }
  , Task::{
      ...
      user = Some {
        ...
        shell = None Text,
        home = Some "{{ sshlogin_src_home | default(omit) }}",
        password_lock = None Bool,
        groups = None ((List Text)),
        append = None Text,
        state = None Text,
        system = None Bool,
        generate_ssh_key = Some True,
        remove = None Text
    },
  ...
  , Task::{
    ...
    in  [
        Entry::{
          name = Some "Create destination user",
          register = Some "_dst_user",
          user = Some {
            name = "{{ sshlogin_dst_user }}"
          , home = Some "{{ sshlogin_dst_home | default(omit) }}"
          , state = None Text
        }
        }
      , Entry::{
