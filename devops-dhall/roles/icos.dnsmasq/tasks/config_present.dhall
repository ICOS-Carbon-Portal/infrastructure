-- Auto-generated from ../../../../devops/roles/icos.dnsmasq/tasks/config_present.yml
...
in  [
    ...
    in  [
        Task::{
          name = Some "Copy dnsmasq config",
          register = Some "config",
          copy = Some {
            content = Some "{{ dnsmasq_config }}"
    ...
    in  [
        Entry::{
          ...
          file = Some { name = Some "{{ config.backup_file }}", state = "absent", path = None Text },
          when = Some "config['backup_file'] is defined"
        }
    ])
    }
  , Task::{
      ...
      systemd = Some {
        ...
        state = Some "{{ 'restarted' if check.changed else 'started' }}",
