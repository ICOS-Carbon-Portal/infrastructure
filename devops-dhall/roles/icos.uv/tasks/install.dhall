-- Auto-generated from ../../../../devops/roles/icos.uv/tasks/install.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of uv",
          register = Some "gh",
          github_release = Some { user = "astral-sh", repo = "uv", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set uv_version fact",
          set_fact = Some {
          ...
          , uv_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        extra_opts = Some [ "--no-same-owner", "--strip-components=1" ],
        mode = None Text,
        creates = None Text
    }
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(uv_version)")
    }
  , Task::{
      name = Some "Which version of uv was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ uv_version }}" })
    }
]
