-- Auto-generated from ../../../../devops/roles/icos.utils/tasks/lazygit.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of lazygit",
          register = Some "gh",
          github_release = Some { user = "jesseduffield", repo = "lazygit", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set lazygit_version fact",
          set_fact = Some {
          ...
          , lazygit_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        owner = Some "root",
        group = Some "root",
        include = Some [ "lazygit" ],
        list_files = None Bool,
        extra_opts = Some [ "--no-same-owner", "--wildcards" ],
        mode = None Text,
        creates = None Text
    }
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not lazygit_version in _r.stdout")
    }
  , Task::{
      name = Some "Which version of lazygit was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ lazygit_version }}" })
    }
]
