-- Auto-generated from ../../../../devops/roles/icos.utils/tasks/fd.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of fd",
          register = Some "gh",
          github_release = Some { user = "sharkdp", repo = "fd", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set fd_version fact",
          set_fact = Some {
          ...
          , fd_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      apt = Some {
        ...
        upgrade = None Text,
        deb = Some "{{ fd_url_map[fd_architecture] }}",
        purge = None Bool,
        autoclean = None Bool,
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        extra_opts = Some [ "--strip-components=1", "--wildcards" ],
        mode = None Text,
        creates = None Text
    }
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(fd_version)")
    }
  , Task::{
