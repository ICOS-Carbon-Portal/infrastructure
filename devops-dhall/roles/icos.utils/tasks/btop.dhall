-- Auto-generated from ../../../../devops/roles/icos.utils/tasks/btop.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of btop",
          register = Some "gh",
          github_release = Some { user = "aristocratos", repo = "btop", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set btop_version fact",
          set_fact = Some {
          ...
          , btop_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      file = Some (Task.Poly_file.Record {
          ...
          name = None Text,
          mode = None Text,
          dest = Some "/usr/local/bin/btop",
          recurse = None Bool,
          src = Some "{{ unarchive.dest }}/btop/bin/btop"
      })
    }
  , Task::{
  ...
  , Task::{
      ...
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ _r.stdout }}" })
    }
]
