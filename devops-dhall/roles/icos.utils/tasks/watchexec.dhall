-- Auto-generated from ../../../../devops/roles/icos.utils/tasks/watchexec.yml
...
in  [
    Task::{
      ...
      file = Some (Task.Poly_file.Record {
          ...
          owner = None Text,
          group = None Text,
          name = Some "/usr/local/sbin/watchexec",
          mode = None Text,
          dest = None Text,
          recurse = None Bool,
          src = None Text
  ...
  , Task::{
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of watchexec",
          register = Some "gr",
          github_release = Some { user = "watchexec", repo = "watchexec", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set watchexec_version fact",
          set_fact = Some {
          ...
          , watchexec_version = Some "{{ gr.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      apt = Some {
        ...
        upgrade = None Text,
        deb = Some "{{ watchexec_url_map[watchexec_architecture] }}",
        purge = None Bool,
        autoclean = None Bool,
  ...
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(watchexec_version)")
    }
  , Task::{
      name = Some "Which version of watchexec was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ watchexec_version }}" })
    }
]
