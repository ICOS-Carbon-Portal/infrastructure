-- Auto-generated from ../../../../devops/roles/icos.utils/tasks/trippy.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of trippy",
          register = Some "gh",
          github_release = Some { user = "fujiapple852", repo = "trippy", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set trippy_version fact",
          set_fact = Some {
          ...
          , trippy_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        extra_opts = Some [ "--strip-components=1" ],
        mode = None Text,
        creates = None Text
    }
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(trippy_version)")
    }
  , Task::{
      name = Some "Which version of trippy was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ trippy_version }}" })
    }
]
