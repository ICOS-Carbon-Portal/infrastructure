-- Auto-generated from ../../../../devops/roles/icos.victoriametrics/tasks/grafana_datasource.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of grafana_datasource",
          register = Some "gh",
          github_release = Some {
      ...
      , Entry::{
          name = Some "Set grafana_datasource_version fact",
          set_fact = Some {
          ...
          , grafana_datasource_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        mode = None Text,
        creates = Some "{{omit if vm_upgrade else vm_graf_plugins + '/victoriametrics-datasource'}}"
    },
      diff = Some False
    }
  , Task::{
      name = Some "Which version of grafana-datasource was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ hostvars.localhost.grafana_datasource_version }}" })
    }
]
