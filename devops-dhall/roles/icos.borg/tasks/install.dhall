-- Auto-generated from ../../../../devops/roles/icos.borg/tasks/install.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of borg",
          register = Some "gh",
          github_release = Some { user = "borgbackup", repo = "borg", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set borg_version fact",
          set_fact = Some {
            borg_version = Some "{{ gh.tag.lstrip('v') }}"
          , cacheable = True
  ...
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(borg_version)")
    }
  , Task::{
      name = Some "Which version of borg is installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ borg_version }}" })
    }
]
