-- Auto-generated from ../../../../devops/roles/icos.restic_server/tasks/install.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of restic_server",
          register = Some "gh",
          github_release = Some { user = "restic", repo = "rest-server", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set restic_server_version fact",
          set_fact = Some {
          ...
          , restic_server_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      unarchive = Some {
        ...
        owner = None Text,
        group = None Text,
        include = Some [ "*/rest-server" ],
        list_files = None Bool,
        extra_opts = Some [ "--no-same-owner", "--strip-components=1", "--wildcards" ],
        mode = Some "+x",
        creates = None Text
    }
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "restic_server_version not in _r.stdout")
    }
  , Task::{
      name = Some "Which version of restic_server was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ restic_server_version }}" })
    }
]
