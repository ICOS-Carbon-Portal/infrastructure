-- Auto-generated from ../../../../devops/roles/icos.flexpart/tasks/flexpart_ssh_user.yml

let Task = ../../../types/Task.dhall

in  [
    Task::{
      name = Some "Run block as {{ _ssh_user }}",
      become = Some (Task.Poly_become.Bool True),
      become_user = Some "{{ _ssh_user }}",
      block = Some (let Task =
        { Type =
            { name : Optional Text
    ...
    in  [
        Task::{
          name = Some "Get home directory of {{ _ssh_user }}",
          register = Some "_home_dir",
          changed_when = Some "False",
          shell = Some "getent passwd {{ _ssh_user }} | cut -d: -f6"
        }
      , Task::{
          name = Some "Make sure user .ssh directory is present",
          file = Some {
            path = Some "{{ _home_dir.stdout }}/.ssh"
          , state = "directory"
          , mode = Some "448"
          , dest = None Text
          , recurse = None Bool
      ...
      , Task::{
          name = Some "Install the rsa private key",
          register = Some "_rsa_file",
          copy = Some {
            content = None Text
          , dest = "{{ _ssh_dir.path}}/flexpart.rsa"
          , backup = None Bool
          , src = Some "roles/icos.flexpart/files/flexpart.rsa"
          , mode = Some 384
        }
        }
      , Task::{
          name = Some "Add flexpart run host to each users known_hosts file",
          known_hosts = Some {
      ...
      , Task::{
          name = Some "Add flexpart users ssh config file",
          blockinfile = Some {
            create = True
