-- Auto-generated from ../../../../devops/roles/icos.docker_utils/tasks/lazydocker.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of lazydocker",
          register = Some "gh",
          github_release = Some { user = "jesseduffield", repo = "lazydocker", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set lazydocker_version fact",
          set_fact = Some {
            borg_version = None Text
          , cacheable = True
          , dive_version = None Text
          , lazydocker_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      file = Some (Task.Poly_file.Record {
          ...
          owner = None Text,
          group = None Text,
          name = Some "/usr/local/sbin/lazydocker",
          mode = None Text,
          dest = None Text,
          recurse = None Bool,
          src = None Text
  ...
  , Task::{
      ...
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ lazydocker_version }}" })
    }
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout_lines[0].endswith(lazydocker_version)")
    }
]
