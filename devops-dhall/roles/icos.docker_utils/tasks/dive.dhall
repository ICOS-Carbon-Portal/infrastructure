-- Auto-generated from ../../../../devops/roles/icos.docker_utils/tasks/dive.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of dive",
          register = Some "gh",
          github_release = Some { user = "wagoodman", repo = "dive", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set dive_version fact",
          set_fact = Some {
            borg_version = None Text
          , cacheable = True
          , dive_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      apt = Some {
        ...
        upgrade = None Text,
        deb = Some "{{ dive_url_map[dive_architecture] }}",
        purge = None Bool,
        autoclean = None Bool,
  ...
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "not _r.stdout.endswith(dive_version)")
    }
  , Task::{
      name = Some "Which version of dive was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ dive_version }}" })
    }
]
