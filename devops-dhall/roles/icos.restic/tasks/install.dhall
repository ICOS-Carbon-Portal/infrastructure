-- Auto-generated from ../../../../devops/roles/icos.restic/tasks/install.yml
...
in  [
    ...
    in  [
        Entry::{
          name = Some "Find the latest release of restic",
          register = Some "gh",
          github_release = Some { user = "restic", repo = "restic", action = "latest_release" }
        }
      , Entry::{
          name = Some "Set restic_version fact",
          set_fact = Some {
          ...
          , restic_version = Some "{{ gh.tag.lstrip('v') }}"
  ...
  , Task::{
      ...
      `ansible.builtin.shell` = Some (Task.Poly_ansible_builtin_shell.Str "curl -L --silent {{ restic_url_map[restic_architecture] }} | bunzip2 > /usr/local/bin/restic && chmod +x /usr/local/bin/restic"),
      args = Some {
        chdir = None Text,
        creates = Some "{{ omit if restic_upgrade else '/usr/local/bin/restic' }}",
  ...
  , Task::{
      ...
      register = Some "_r",
      failed_when = Some (Task.Poly_failed_when.Str "restic_version not in _r.stdout")
    }
  , Task::{
      name = Some "Which version of restic was installed",
      debug = Some (Task.Poly_debug.Record { msg = "Installed {{ restic_version }}" })
    }
]
